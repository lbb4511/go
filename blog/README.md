...
4. go 学习难点:
    ...
    11. go channel和gorutone使用方法；并发编程模式；实现原理. 如果形容go语言是一个皇冠，goroutine 和channel则是皇冠上的明珠，go是CSB并发编程理论的实践者和改良派；go将并发编程的复杂性降低了一个数量级，从此并发编程不再是某些自称“技术淫人”的专利；世界本应该如此简单，语言和编译器能做好的事情就不要让程序员瞎忙活，这是go的哲学；
